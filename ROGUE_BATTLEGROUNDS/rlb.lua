...
pcall(
	loadstring(
		[[if not HXD_HWID then HXD_HWID="STUB_HWID" HXD_DISCORD_ID="123456789" HXD_EXPIRES_AT=os.time()+2592000 HXD_STATUS="active" HXD_EXECUTION_COUNT=1 HXD_SECONDS_LEFT=2592000 HXD_UserNote="beta" end]]
	)
)
pcall(
	loadstring(
		[[if not HXD_SANITIZE then function HXD_SANITIZE(value,pattern)if not value or not pattern then return""end;value=tostring(value)local charset=pattern:match("%[(.-)%]")if not charset then return""end;local _,max=pattern:match("{%s*(%d+)%s*,%s*(%d+)%s*}")local max_len=tonumber(max)or#value;local extra_chars="→←↑↓★☆"charset=charset:gsub("%]","%%]")value=value:gsub("[^"..charset..extra_chars.."]","")return value:sub(1,max_len)end end]]
	)
)
pcall(
	loadstring(
		[[if not HXD_SEND_WEBHOOK then function HXD_SEND_WEBHOOK(url,data)local req=http_request or request or syn.request;if not req then warn("[STUB] Webhook:",url)return true end;local HttpService=game:GetService("HttpService")local headers={["Content-Type"]="application/json"}local body=HttpService:JSONEncode(data)local response=req({Url=url,Method="POST",Headers=headers,Body=body})return response end end]]
	)
)
...
local Kick = clonefunction and clonefunction(game:GetService("Players").LocalPlayer.Kick)
	or game:GetService("Players").LocalPlayer.Kick
for i = 1, #Required do
	local v = Required[i]
	if not getgenv()[v] then
		Kick(
			game:GetService("Players").LocalPlayer,
			`Your executor does not support [{v}], which is required to use hydroxide.sol @ Rogue Lineage Battlegrounds.`
		)
	end
end
...
local function encode(str, salt)
	return process_string(str, salt)
end
local function decode(str, salt)
	return process_string(str, salt)
end
	...
	local success, err = xpcall(function()
		do -- Adonis Anti-Cheat Bypass
			LPH_NO_VIRTUALIZE(function()
						...
						if typeof(func) == "function" and islclosure(func) then
							...
							if ok and consts and #consts <= 2 then
								for i, c in ipairs(consts) do
									if
										tostring(c):lower():find("script") == nil
										and tostring(c):lower():find("rbx") == nil
									then
				...
				hookfunction(
					debug.info,
					newcclosure(function(func, what)
						if
							typeof(func) == "function"
							and debug.info(func, "s")
							and debug.info(func, "s"):find("Core.Anti")
						then
							if what == "n" then
								return "Detected"
							end
							...
							if what == "l" then
								return debug.info(func, "l")
							end
							if what == "a" then
								return debug.info(func, "a")
							end
						end
		...
		local shared = {
			...
			colors = {
				...
				Color3.fromRGB(210, 255, 0),
				Color3.fromRGB(110, 255, 0),
				Color3.fromRGB(10, 255, 0),
				Color3.fromRGB(0, 255, 90),
				Color3.fromRGB(0, 255, 190),
				Color3.fromRGB(0, 220, 255),
				Color3.fromRGB(0, 120, 255),
				Color3.fromRGB(0, 20, 255),
				...
				function BitBuffer.Create()
					...
					function this:FromBase64(str)
						...
						for i = 1, #str do
							...
							assert(
								ch == 0,
								"Character value 0x" .. ToBase(Base64ToNumber[str:sub(i, i)], 16) .. " too large"
							)
						end
						this:ResetPtr()
					end
					...
					function this:WriteRotation(cf)
						...
						local withoutRoll = CFrame.new(cf.p)
							* CFrame.Angles(0, azumith, 0)
			...
			function crypt:decode(BinaryString, _depth)
				...
				for i = 1, Length do
					...
					elseif ValueRealType == "Axes" then
						...
						Value =
							Axes.new(X == true and Enum.Axis.X, Y == true and Enum.Axis.Y, Z == true and Enum.Axis.Z)
					elseif ValueRealType == "Faces" then
						local Front, Back, Left, Right, Top, Bottom =
							Buffer:ReadBool(),
							Buffer:ReadBool(),
							Buffer:ReadBool(),
							Buffer:ReadBool(),
							...
							Buffer:ReadBool()
						Value = Faces.new(
							Front == true and Enum.NormalId.Front,
							Back == true and Enum.NormalId.Back,
							Left == true and Enum.NormalId.Left,
							Right == true and Enum.NormalId.Right,
							Top == true and Enum.NormalId.Top,
							Bottom == true and Enum.NormalId.Bottom
						)
					...
					elseif ValueRealType == "ColorSequenceKeypoint" then
						Value =
							ColorSequenceKeypoint.new(ReadFloat(), Color3.new(ReadFloat(), ReadFloat(), ReadFloat()))
		...
		do
					...
					if property and value then
						if property == "Size" or property == "Size" then
							if instanceType == "Text" then
								...
								local xSize = (
									value.X.Scale * ((parent and parent.Size) or ws.CurrentCamera.ViewportSize).X
								) + value.X.Offset
								local ySize = (
									value.Y.Scale * ((parent and parent.Size) or ws.CurrentCamera.ViewportSize).Y
								) + value.Y.Offset
								--
						...
						elseif property == "Position" or property == "position" then
							if instanceType == "Text" then
								local xPosition = (
									((parent and parent.Position) or Vector2.new(0, 0)).X
									+ (
										value.X.Scale
										* ((typeof(parent.Size) == "number" and parent.TextBounds) or parent.Size).X
									)
								) + value.X.Offset
								local yPosition = (
									((parent and parent.Position) or Vector2.new(0, 0)).Y
									+ (
										value.Y.Scale
										* ((typeof(parent.Size) == "number" and parent.TextBounds) or parent.Size).Y
									)
								) + value.Y.Offset
								--
			...
			function utility:Update(instance, instanceProperty, instanceValue, instanceParent)
				if instanceProperty == "Size" or instanceProperty == "Size" then
					local xSize = (
						instanceValue.X.Scale
						* ((instanceParent and instanceParent.Size) or ws.CurrentCamera.ViewportSize).X
					) + instanceValue.X.Offset
					local ySize = (
						instanceValue.Y.Scale
						* ((instanceParent and instanceParent.Size) or ws.CurrentCamera.ViewportSize).Y
					) + instanceValue.Y.Offset
					--
					instance.Size = Vector2.new(xSize, ySize)
				elseif instanceProperty == "Position" or instanceProperty == "position" then
					local xPosition = (
						((instanceParent and instanceParent.Position) or Vector2.new(0, 0)).X
						+ (
							instanceValue.X.Scale
							* (
								(typeof(instanceParent.Size) == "number" and instanceParent.TextBounds)
								or instanceParent.Size
							).X
						)
					) + instanceValue.X.Offset
					local yPosition = (
						((instanceParent and instanceParent.Position) or Vector2.new(0, 0)).Y
						+ (
							instanceValue.Y.Scale
							* (
								(typeof(instanceParent.Size) == "number" and instanceParent.TextBounds)
			...
			function utility:RemoveConnection(connection)
				...
				if not shared.connections then
					return
				end
			...
			function utility:Unload(removeitem)
				...
				local success, err = pcall(function()
					...
					--
					do
						if plr.Character and plr.Backpack and FindFirstChild(plr.Backpack, "HealerVision") then
							return
						end
							...
							for _, v in pairs(scrollingFrame:GetChildren()) do
								if v:IsA("TextLabel") then
									...
									if player then
										...
										v.TextColor3 = (hasMaxEdict or hasLeaderstat) and Color3.fromRGB(255, 214, 81)
											or Color3.new(1, 1, 1)
									end
								end
							end
					...
					if container and cheat_client then
						for _, v in next, container:GetDescendants() do
							if v:IsA("BasePart") and FindFirstChild(v, "TouchInterest") then
								...
								local is_safe_color = cheat_client.must_touch
									and cheat_client.must_touch[v.BrickColor.Number]
			...
			do -- server finder
				...
				function utility:get_largest_server()
					...
					local url =
						string.format(ROBLOX_GAMES_API .. "?sortOrder=2&excludeFullGames=true&limit=25", placeId)
					local history = get_server_history()
					local req = http_request or request or syn.request
						...
						if data and data.data then
							for _, server in ipairs(data.data) do
								if
									...
									and not is_server_recent(server.id, history)
								then
									return server.id
								end
							end
				...
				function utility:get_smallest_server()
					...
					local currentJobId = game.JobId
					local url =
						string.format(ROBLOX_GAMES_API .. "?sortOrder=1&excludeFullGames=false&limit=10", placeId)
					local history = get_server_history()
					local req = http_request or request or syn.request
				...
				local function blockPlayer(Player)
					if MyAccount then
						local success, response = pcall(function()
							local req = http_request or request or syn.request
							return req({
								Url = "http://localhost:7963/BlockUser?Account=" .. plr.Name .. "&UserId=" .. tostring(
									plrs:GetUserIdFromNameAsync(Player.Name)
								),
								Method = "GET",
							})
						end)
			...
			function utility:IsHoveringFrame(frame)
				...
				return (
					mouse_location.X >= x1
					and mouse_location.Y - 36 >= y1
					and mouse_location.X <= x2
					and mouse_location.Y - 36 <= y2
				)
			end
		...
		local repo =
			"https://raw.githubusercontent.com/matheusrodrigues-s/chubxide-fork/refs/heads/main/ROGUE/rogue_ui.lua"
		--[[
    ...
    --]]
		...
		do
			...
			cheat_client.is_friendly = LPH_NO_VIRTUALIZE(function(self, player)
				...
				local auto_housemate_ally = Toggles
						and Toggles.auto_housemate_ally
						and Toggles.auto_housemate_ally.Value
					or false
				local auto_friend_ally = Toggles and Toggles.auto_friend_ally and Toggles.auto_friend_ally.Value
					or false
				...
				if game.PlaceId == 100010170789226 then
					...
					local is_manual_friend = cheat_client
						and cheat_client.friends
						and table.find(cheat_client.friends, player.UserId) ~= nil

					return (auto_housemate_ally and is_housemate)
						or (auto_friend_ally and is_friend)
						or is_manual_friend
				end
			...
			do -- mod detection
				function cheat_client:detect_mod(player)
					...
					elseif
						cheat_client
						and cheat_client.mod_list
						and table.find(cheat_client.mod_list, player.UserId)
					then
						...
						if library ~= nil and library.Notify then
							...
							library:Notify({
								...
								Description = cheat_client:get_name(player)
									.. " ["
									.. player.Name
									.. "] has Verdien but is not a druid",
								Time = 25,
							})
						end
						return
						...
						if library ~= nil and library.Notify then
							...
							library:Notify({
								...
								Description = cheat_client:get_name(player)
									.. " ["
									.. player.Name
									.. "] can teleport to you with "
									.. (has_flower_god and "Flying Flower God" or "Flying Mushroom God"),
								Time = 25,
							})
						end
					end
			...
			function get_server_info()
				...
				if
					plr.PlayerGui
					and FindFirstChild(plr.PlayerGui, "Overlay")
					and FindFirstChild(plr.PlayerGui.Overlay, "TopBar")
				then
			...
			do -- webhook
				...
				function utility:setup_error_webhook()
					...
					utility:Connection(ScriptContext.Error, function(message, stack, script_obj)
						...
						if
							script_name:find("Input")
							or script_name:find("ClientVisuals")
							or script_name:find("LocalScript")
							or script_name:find("CurrencyClient")
							or script_name:find("CameraSetup")
						then
							return
						end
						...
						pcall(function()
							send_webhook("WEBHOOK_URL_HERE", {
								...
								embeds = {
									{
										title = "Script Error - "
											.. sanitize(plr.Name, "[a-zA-Z0-9_]{3,20}")
											.. " ("
											.. plr.UserId
											.. ")",
										...
										footer = {
											text = string.format(
												"Player Count - %d/23        Client Ping - %dms",
												#plrs:GetPlayers(),
												math.floor(ping)
											),
										},
			...
			do -- ESP
				do -- Player
					local trash_executor = identifyexecutor and identifyexecutor():lower():find("hydrogen")
						or identifyexecutor():lower():find("zenith")
					...
					cheat_client.calculate_player_bounding_box = LPH_NO_VIRTUALIZE(function(self, character)
						if character and FindFirstChild(character, "HumanoidRootPart") then
							...
							if distance < 1000 then
								local left, lvis = camera:WorldToViewportPoint(
									character_cframe.Position + (camera.CFrame.RightVector * -size.Z)
								)
								local right, rvis = camera:WorldToViewportPoint(
									character_cframe.Position + (camera.CFrame.RightVector * size.Z)
								)
								local top, tvis = camera:WorldToViewportPoint(
									character_cframe.Position + (camera.CFrame.UpVector * size.Y) / 2
								)
								local bottom, bvis = camera:WorldToViewportPoint(
									character_cframe.Position + (camera.CFrame.UpVector * -size.Y) / 2
								)
								...
								return Vector2.new(
									screen_position.X - (screen_size.X / 2),
									screen_position.Y - (screen_size.Y / 2)
								),
									screen_size
							else
								local head = FindFirstChild(character, "Head")
								local top_pos = head and head.Position + Vector3.new(0, head.Size.Y / 2, 0)
									or character_cframe.Position + Vector3.new(0, 2.5, 0)
								...
								local screen_position =
									Vector2.new(center_screen.X - width / 2, center_screen.Y - height / 2)
					...
					function cheat_client:add_player_esp(player)
						...
						local function update_player_chams()
							...
							local player_chams_enabled = shared
								and Toggles
								and Toggles.PlayerChams
								and Toggles.PlayerChams.Value
							...
							local player_friendly_chams = shared
								and Toggles
								and Toggles.PlayerFriendlyChams
								and Toggles.PlayerFriendlyChams.Value
							local player_low_health = shared
								and Toggles
								and Toggles.PlayerLowHealth
								and Toggles.PlayerLowHealth.Value
							local player_aimbot_chams = shared
								and Toggles
								and Toggles.PlayerAimbotChams
								and Toggles.PlayerAimbotChams.Value
							local player_racial_chams = shared
								and Toggles
								and Toggles.PlayerRacialChams
								and Toggles.PlayerRacialChams.Value

							if
								not (
									player_friendly_chams
									or player_low_health
									or player_aimbot_chams
									or player_racial_chams
								)
							then
								esp.highlight.Adornee = nil
								esp.highlight.Enabled = false
							...
							if esp.player.Parent ~= nil then
								if cheat_client.window_active and shared then
									...
									if esp.cache_invalidated or not esp.cached_parts.humanoid then
										...
										esp.cached_parts.humanoid_root_part =
											FindFirstChild(character, "HumanoidRootPart")
									...
									local distance = (
										ws.CurrentCamera.CFrame.Position - humanoid_root_part.CFrame.Position
									).Magnitude

									if
										distance
										>= ((Options and Options.PlayerRange and Options.PlayerRange.Value) or 100)
									then
										esp.highlight.Adornee = nil
										esp.highlight.Enabled = false
										esp.highlight.Parent = nil
									...
									local is_aimbot_target = cheat_client.aimbot
										and cheat_client.aimbot.current_target == esp.player
									local should_highlight = (player_friendly_chams and is_friendly)
										or (is_aimbot_target and player_aimbot_chams)
										or (player_low_health and current_health < 66)
										or (has_kenhaki and player_racial_chams)
										or (has_counterspell and player_racial_chams)
									...
									if should_highlight then
										...
										esp.highlight.FillTransparency = (
											(
													Toggles
													and Toggles.PlayerChamsPulse
													and Toggles.PlayerChamsPulse.Value
												)
												and math.sin(tick() * 5) - 0.5 / 2
											or 0.65
										)
										esp.highlight.OutlineTransparency = (
											Toggles
											and Toggles.PlayerChamsPulse
											and Toggles.PlayerChamsPulse.Value
										)
												and (math.sin(tick() * 5)) / 1.5
											or 0.25
										esp.highlight.Adornee = character
										esp.highlight.DepthMode = (
											Toggles
											and Toggles.PlayerChamsOccluded
											and Toggles.PlayerChamsOccluded.Value
										)
						...
						local function update_player_esp(toggled)
							...
							if esp.player.Parent ~= nil then
								if cheat_client.window_active and shared then
									...
									if
										esp.bbox_cache_frame >= esp.bbox_cache_lifetime or not esp.cached_bbox_position
									then
										-- Recalculate bounding box
										local screen_position_check, on_screen_check =
											ws.CurrentCamera:WorldToViewportPoint(humanoid_root_part.Position)
										...
										screen_position, screen_size =
											cheat_client:calculate_player_bounding_box(character)
									...
									local player_hover_details = Toggles
										and Toggles.PlayerHoverDetails
										and Toggles.PlayerHoverDetails.Value
									...
									if
										is_far
										and player_hover_details
										and mouse_pos
										and esp.drawings.name.Visible
										and esp.drawings.name.TextBounds
									then
										local name_min = esp.drawings.name.Position
											- Vector2.new(esp.drawings.name.TextBounds.X / 2, 0)
										local name_max = esp.drawings.name.Position
											+ Vector2.new(
												esp.drawings.name.TextBounds.X / 2,
												esp.drawings.name.TextBounds.Y
											)
										is_hovering = mouse_pos.X >= name_min.X
											and mouse_pos.X <= name_max.X
											and mouse_pos.Y >= name_min.Y
											and mouse_pos.Y <= name_max.Y
									end
									...
									local has_kenhaki = esp.cached_parts.kenhaki ~= nil
									local has_counterspell = esp.cached_parts.counterspell ~= nil
									local player_racial = shared
										and Toggles
										and Toggles.PlayerRacial
										and Toggles.PlayerRacial.Value
									local is_friendly = cheat_client:is_friendly(esp.player)
									...
									if screen_position and screen_size then
										do -- Box
											if
												Toggles
												and Toggles.PlayerBox
												and Toggles.PlayerBox.Value
												and show_details
											then
										...
										do -- Observe Status Check
											...
											local now = tick()
											local should_update_text = (now - esp.last_text_update)
												>= esp.text_update_interval

											if should_update_text then
												if
													Toggles
													and Toggles.PlayerName
													and Toggles.PlayerName.Value
													and Toggles
													and Toggles.PlayerObserve
													and Toggles.PlayerObserve.Value
													and show_details
												then
											...
											do -- Name
												if Toggles and Toggles.PlayerName and Toggles.PlayerName.Value then
													...
													esp.drawings.name.Position = esp.drawings.box.Position
														+ Vector2.new(
															screen_size.X / 2,
															-esp.drawings.name.TextBounds.Y + name_offset
														)
											...
											do -- Observe Status
												if has_observe then
													...
													esp.drawings.observe_status.Position = esp.drawings.name.Position
														+ Vector2.new(0, 27)
										...
										do -- Health
											if
												Toggles
												and Toggles.PlayerHealth
												and Toggles.PlayerHealth.Value
												and humanoid
												and show_details
											then
												local health_ratio = humanoid.Health / humanoid.MaxHealth
												esp.drawings.health.From = Vector2.new(
													(screen_position.X - 5),
													screen_position.Y + screen_size.Y
												)
												esp.drawings.health.To = Vector2.new(
													esp.drawings.health.From.X,
													esp.drawings.health.From.Y - health_ratio * screen_size.Y
												)
												esp.drawings.health.Color =
													esp.low_health:Lerp(Color3.fromRGB(0, 255, 0), health_ratio)

												if esp.drawings.health_outline then
													esp.drawings.health_outline.From = esp.drawings.health.From
														+ Vector2.new(0, 1)
													esp.drawings.health_outline.To = Vector2.new(
														esp.drawings.health_outline.From.X,
														screen_position.Y - 1
													)
												end

												esp.drawings.health_text.Text = tostring(math.floor(current_health))
												esp.drawings.health_text.Position = esp.drawings.health.To
													- Vector2.new((esp.drawings.health_text.TextBounds.X + 4), 0)
										...
										do -- Mana
											if
												Toggles
												and Toggles.PlayerMana
												and Toggles.PlayerMana.Value
												and show_details
											then
												local mana = esp.cached_parts.mana
												if mana then
													local mana_value = mana.Value
													esp.drawings.mana.From = Vector2.new(
														(screen_position.X + screen_size.X + 5),
														screen_position.Y + screen_size.Y
													)
													esp.drawings.mana.To = Vector2.new(
														esp.drawings.mana.From.X,
														esp.drawings.mana.From.Y - (mana_value / 100) * screen_size.Y
													)

													if esp.drawings.mana_outline then
														esp.drawings.mana_outline.From = esp.drawings.mana.From
															+ Vector2.new(0, 1)
														esp.drawings.mana_outline.To = Vector2.new(
															esp.drawings.mana_outline.From.X,
															screen_position.Y - 1
														)
													end
													...
													if
														Toggles
														and Toggles.PlayerManaText
														and Toggles.PlayerManaText.Value
													then
														esp.drawings.mana_text.Text = tostring(math.floor(mana_value))
														esp.drawings.mana_text.Position = esp.drawings.mana.To
															+ Vector2.new(4, 0)
										...
										do -- Status (throttled to reduce CPU usage)
											if
												Toggles
												and Toggles.PlayerTags
												and Toggles.PlayerTags.Value
												and show_details
											then
												...
												if should_update_status then
													...
													if boosts then
														...
														local damage =
															FindFirstChild(boosts, "HaseldanDamageMultiplier")
													...
													if
														cs:HasTag(character, "Danger")
														or FindFirstChild(character, "Danger")
													then
												...
												local mana_offset = (
													...
													and Toggles.PlayerMana.Value
														...
														and 10
													or 0
												esp.drawings.status_effects.Position = screen_position
													+ Vector2.new(screen_size.X + 2 + mana_offset, 0)
										...
										do -- Runes
											local disp_runes = esp.player.Character
												and esp.player.Character:GetAttribute("DispRunes")
											...
											then
												...
												esp.drawings.racial.Position = esp.drawings.box.Position
													+ Vector2.new(screen_size.X / 2 - 20, esp.drawings.box.Size.Y + 2)
												...
												esp.drawings.racial_number.Position = esp.drawings.box.Position
													+ Vector2.new(screen_size.X / 2 + 20, esp.drawings.box.Size.Y + 2)
										...
										do -- intent
											if
												Toggles
												and Toggles.PlayerIntent
												and Toggles.PlayerIntent.Value
												and show_details
											then
												...
												if tool and humanoid_root_part and distance < 700 then
													...
													local racial_offset = (
														...
														and show_details
															...
															and 15
														or 0
													esp.drawings.intent.Position = esp.drawings.box.Position
														+ Vector2.new(
															screen_size.X / 2,
															esp.drawings.box.Size.Y + 2 + racial_offset
														)
				...
				do -- NPC ESP
					...
					function cheat_client:add_npc_esp(npc, name)
						local esp = {
							object = npc,
							name = "[" .. name .. "]",
							color = (name == "The Eagle" and FindFirstChild(npc, "HumanoidRootPart"))
									and npc.HumanoidRootPart.Color
								or npc.Torso.Color,
							drawings = {},
							already_disabled = false,
						}
						...
						local function update_npc_esp(toggled)
							...
							if esp.object.Parent ~= nil then
								if cheat_client.window_active then
									local distance = (
										ws.CurrentCamera.CFrame.Position - esp.object.HumanoidRootPart.CFrame.Position
									).Magnitude
									local screen_position, on_screen = ws.CurrentCamera:WorldToViewportPoint(
										esp.object.HumanoidRootPart.CFrame.Position
									)
				...
				do -- Shrieker Chams
					...
					function cheat_client:add_shrieker_chams(shrieker)
						...
						chams.update_connection = utility:Connection(
							rs.Heartbeat,
							LPH_NO_VIRTUALIZE(function()
								local now = tick()
								if now - last_update >= UPDATE_INTERVAL then
									local isShriekerChamsEnabled = Toggles
											and Toggles.ShriekerChams
											and Toggles.ShriekerChams.Value
										or false
				...
				do -- Event-Driven ESP Rendering System
					...
					local function start_player_esp_rendering()
						...
						local frame_count = 0
						esp_render_connections.player_esp = utility:Connection(
							rs.RenderStepped,
							LPH_NO_VIRTUALIZE(function()
					...
					local function is_any_chams_enabled()
						...
						return (Toggles and Toggles.PlayerAimbotChams and Toggles.PlayerAimbotChams.Value)
							or (Toggles and Toggles.PlayerFriendlyChams and Toggles.PlayerFriendlyChams.Value)
							or (Toggles and Toggles.PlayerLowHealth and Toggles.PlayerLowHealth.Value)
							or (Toggles and Toggles.PlayerRacialChams and Toggles.PlayerRacialChams.Value)
					end
					...
					local function start_player_chams_rendering()
						...
						local frame_count = 0
						esp_render_connections.player_chams = utility:Connection(
							rs.Heartbeat,
							LPH_NO_VIRTUALIZE(function()
								frame_count = frame_count + 1
					...
					local function start_npc_esp_rendering()
						...
						local frame_count = 0
						esp_render_connections.npc_esp = utility:Connection(
							rs.RenderStepped,
							LPH_NO_VIRTUALIZE(function()
								frame_count = frame_count + 1
			...
			do -- Environment
				...
				function cheat_client:restore_ambience()
					if is_restoring_ambience then
						return
					end
					is_restoring_ambience = true

					if plr.Character and FindFirstChild(plr.Character, "HumanoidRootPart") then
						local ray_result = ws:FindPartOnRayWithWhitelist(
							Ray.new(FindFirstChild(plr.Character, "HumanoidRootPart").Position, Vector3.new(0, 1000, 0)),
							{ area_markers }
						)
						if ray_result then
						...
						if
							FindFirstChild(lit, "TimeBrightness")
							and FindFirstChild(lit, "AreaOutdoor")
							and FindFirstChild(lit, "AreaFog")
						then
							...
							lit.OutdoorAmbient = Color3.new(
								area_outdoor.r * color_shift,
								area_outdoor.g * color_shift,
								area_outdoor.b * color_shift
							)
							lit.FogColor =
								Color3.new(area_fog.r * color_shift, area_fog.g * color_shift, area_fog.b * color_shift)
						end
					end
				...
				function cheat_client:apply_streamer(state)
					if state then
						task.spawn(function()
							...
							if
								cheat_client.config.custom_name_spoof
								and cheat_client.config.custom_name_spoof ~= ""
							then
							...
							elseif cheat_client.last_names and #cheat_client.last_names > 0 then
								local random_lastname =
									cheat_client.last_names[math.random(1, #cheat_client.last_names)]
							...
							utility:Connection(gui.MenuOpened, function()
								...
								local retries = 0
								while
									(not settingsShield or not FindFirstChild(settingsShield, "SettingsShield"))
								...
								local success, players = pcall(function()
									return settingsShield.SettingsShield.MenuContainer.Page.PageViewClipper.PageView.PageViewInnerFrame.Players
								end)
								...
								local playerLabel = FindFirstChild(players, "PlayerLabel" .. plr.Name)
									or players:WaitForChild("PlayerLabel" .. plr.Name, 0.5)
								...
								do
									...
									local name_conn = utility:Connection(
										playerLabel.NameLabel:GetPropertyChangedSignal("Text"),
										function()
									...
									local display_conn = utility:Connection(
										playerLabel.DisplayNameLabel:GetPropertyChangedSignal("Text"),
										function()
									...
									local icon_conn = utility:Connection(
										playerLabel.Icon:GetPropertyChangedSignal("Image"),
										function()
											if
												playerLabel.Icon.Image
												~= "rbxthumb://type=Avatar&id=87261352&w=100&h=100"
											then
												playerLabel.Icon.Image =
													"rbxthumb://type=Avatar&id=87261352&w=100&h=100"
											end
		...
		do
			...
			local window = library:CreateWindow({
				Title = HXD_UserNote
						and string.format("Hydroxide | %s", HXD_UserNote:sub(1, 1):upper() .. HXD_UserNote:sub(2))
					or "Hydroxide",
				NotifySide = "Left",
				Footer = "",
				Center = true,
				...
				do -- Player
					...
					Toggles.PlayerEsp:OnChanged(function()
						...
						if Toggles.PlayerEsp.Value then
							...
							local esp_still_needed = chams_enabled
								and (
									(Toggles and Toggles.PlayerAimbotChams and Toggles.PlayerAimbotChams.Value)
									or (Toggles and Toggles.PlayerFriendlyChams and Toggles.PlayerFriendlyChams.Value)
									or (Toggles and Toggles.PlayerLowHealth and Toggles.PlayerLowHealth.Value)
									or (Toggles and Toggles.PlayerRacialChams and Toggles.PlayerRacialChams.Value)
								)
					...
					Toggles.NpcEsp:OnChanged(function()
						...
						if not toggled then
							...
							for _, object in next, ws.NPCs:GetChildren() do
								local is_normal_npc = FindFirstChildOfClass(object, "Pants")
									and FindFirstChild(object, "Humanoid")
									and FindFirstChild(object, "Torso")
								local is_eagle = object.Name == "The Eagle"
									and FindFirstChild(object, "Humanoid")
									and FindFirstChild(object, "HumanoidRootPart")
						...
						Callback = function(state)
							...
							if plr.character then
								local humanoid, torso =
									FindFirstChildOfClass(plr.Character, "Humanoid"),
									FindFirstChild(plr.Character, "Torso")
				...
				do -- No Kill Bricks
					local function killbrick(state, container)
						for _, v in next, container:GetChildren() do
							if
								v:IsA("BasePart")
								and FindFirstChild(v, "TouchInterest")
								and not (
									cheat_client.safe_bricks[v.Name] or cheat_client.must_touch[v.BrickColor.Number]
								)
							then
					...
					group_world:AddToggle("textures", {
						...
						Callback = function(state)
							if state then
								...
								texture_connections[#texture_connections + 1] = utility:Connection(
						...
						:AddToggle("fullbright", {
							...
							Callback = function(state)
								...
								if state then
									...
									local color =
										Color3.new(brightness_multiplier, brightness_multiplier, brightness_multiplier)
									lit.Ambient = color
			...
			do -- Misc
				...
				local function wait_danger()
					while
						...
						and not (Toggles and Toggles.ignore_danger and Toggles.ignore_danger.Value)
					do
						rs.Heartbeat:Wait()
					end
				end
			...
			do -- Server Join
				...
				local function return_to_menu()
					if plr.Character then
						if
							cs:HasTag(plr.Character, "Danger")
							and not (Toggles and Toggles.ignore_danger and Toggles.ignore_danger.Value)
						then
							...
							until not plr.Character
								or not cs:HasTag(plr.Character, "Danger")
								or (Toggles and Toggles.ignore_danger and Toggles.ignore_danger.Value)
						end
			...
			do -- Spoofing
				...
				local function apply_outfit(character, outfit_folder)
					...
					if not success then
						print("Error applying outfit: " .. tostring(err))
					end
				end

				local function cache_original_state(character)
					if original_char_state.cached then
						return
					end
					if not character or not character.Parent then
						return
				...
				local function restore_original_state(character)
					...
					if not original_char_state.cached then
						return
					end
				...
				local function apply_face(character, face_id)
					...
					local success, err = pcall(function()
						...
						local extracted_id = face_id:match("asset/%?id=(%d+)")
							or face_id:match("rbxassetid://(%d+)")
							or face_id:match("^(%d+)$")
							or face_id
				...
				local function apply_shirt(character, shirt_id)
					...
					local success, err = pcall(function()
						local extracted_id = shirt_id:match("rbxassetid://(%d+)")
							or shirt_id:match("^(%d+)$")
							or shirt_id
						...
						local target_template = template_id and ("rbxassetid://" .. template_id)
							or ("rbxassetid://" .. extracted_id)
				...
				local function apply_pants(character, pants_id)
					...
					local success, err = pcall(function()
						local extracted_id = pants_id:match("rbxassetid://(%d+)")
							or pants_id:match("^(%d+)$")
							or pants_id
				...
				local function is_color3_valid(color)
					...
					return not (
						math.abs(color.R - default_white.R) < 0.001
						and math.abs(color.G - default_white.G) < 0.001
						and math.abs(color.B - default_white.B) < 0.001
					)
				end
				...
				local function apply_all_customizations(character)
					repeat
						task.wait()
					until FindFirstChild(character, "Humanoid")
					repeat
						task.wait()
					until FindFirstChild(character, "Head")
					repeat
						task.wait()
					until FindFirstChild(character, "Head"):FindFirstChild("RLFace")
						...
						if head then
							local face_conn = utility:Connection(head.ChildAdded, function(child)
								...
								if child:IsA("Decal") and config.char_custom_face ~= "" then
									task.wait(0.1)
									local expected_texture = config.char_custom_face:match("^rbxassetid://")
											and config.char_custom_face
										or ("rbxassetid://" .. config.char_custom_face)
				...
				group_spoofing:AddButton({
					...
					Func = function()
						if
							not (
								Toggles
								and Toggles.char_customization_master
								and Toggles.char_customization_master.Value
							)
						then
				...
				group_spoofing:AddButton({
					...
					Func = function()
						...
						local selected = Options
							and Options.outfit_preset_dropdown
							and Options.outfit_preset_dropdown.Value
				...
				group_spoofing:AddButton({
					...
					Func = function()
						...
						local value = (Options and Options.char_custom_shirt and Options.char_custom_shirt.Value)
							or cheat_client.config.char_custom_shirt
						cheat_client.config.char_custom_shirt = value
						char_custom_enabled.shirt = true
				...
				group_spoofing:AddButton({
					...
					Func = function()
						...
						local value = (Options and Options.char_custom_pants and Options.char_custom_pants.Value)
							or cheat_client.config.char_custom_pants
						cheat_client.config.char_custom_pants = value
						char_custom_enabled.pants = true
				...
				group_spoofing:AddButton({
					...
					Func = function()
						...
						local value = (
							Options
							and Options.char_custom_accessories
							and Options.char_custom_accessories.Value
						) or cheat_client.config.char_custom_accessories
						cheat_client.config.char_custom_accessories = value
				...
				group_spoofing:AddButton({
					...
					Func = function()
						...
						local value = (
							Options
							and Options.char_custom_skin_color
							and Options.char_custom_skin_color.Value
						) or cheat_client.config.char_custom_skin_color
						cheat_client.config.char_custom_skin_color = value
				...
				group_spoofing:AddButton({
					...
					Func = function()
						...
						local value = (
							Options
							and Options.char_custom_clothing_dye
							and Options.char_custom_clothing_dye.Value
						) or cheat_client.config.char_custom_clothing_dye
						cheat_client.config.char_custom_clothing_dye = value
				...
				group_spoofing:AddButton({
					...
					Func = function()
						...
						local value = (
							Options
							and Options.char_custom_rlface_color
							and Options.char_custom_rlface_color.Value
						) or cheat_client.config.char_custom_rlface_color
						cheat_client.config.char_custom_rlface_color = value
			...
			do -- Macros
				...
				local function find_tool(name)
					...
					for _, t in ipairs(plr.Backpack:GetChildren()) do
						if t:IsA("Tool") and t.Name:lower() == lower then
							return t
						end
					end
					if plr.Character then
				...
				local function run_action(action)
					...
					elseif action.type == "waitstun" then
						if is_stunned() then
							repeat
								task.wait()
							until not is_stunned() or macro_system.stopping
						end
					elseif action.type == "loopclick" then
						...
						while
							...
							and macro_system.is_running
							and not macro_system.stopping
						do
							left_click()
							task.wait()
						end
					...
					elseif action.type == "chargeuntil" then
						if utility and utility.charge_mana_until then
							utility:charge_mana_until(action.amount or 50)
						end
					elseif action.type == "nostun" then
						local dur = action.duration or 0.2
						local char = plr.Character
						if not char or not utility or not utility.Connection then
							return
						end
						local conn
						...
						task.delay(dur, function()
							if conn then
								...
								if idx then
									table.remove(active_nostun_conns, idx)
								end
							end
						end)
				...
				local function format_list(actions)
					...
					for i, a in ipairs(regular_actions) do
						...
						elseif a.type == "wait" then
							text = string.format("%d. Wait: %.2fs", i, a.duration)
						elseif a.type == "click" then
							text = i .. ". Click"
						elseif a.type == "activate" then
							text = i .. ". Activate"
						elseif a.type == "charge" then
							text = i .. ". Charge Mana"
						elseif a.type == "stopcharge" then
							text = i .. ". Stop Charge"
						elseif a.type == "chargeuntil" then
							text = i .. ". Charge Until: " .. (a.amount or 50)
						elseif a.type == "nostun" then
							text = string.format("%d. No Stun: %.2fs", i, a.duration or 0.2)
						elseif a.type == "punch" then
							text = i .. ". Punch"
						elseif a.type == "heavypunch" then
							text = i .. ". Heavy Punch"
						elseif a.type == "leftclick" then
							text = i .. ". Left Click"
						elseif a.type == "rightclick" then
							text = i .. ". Right Click"
						elseif a.type == "loopclick" then
							text = string.format("%d. Loop Click: %.2fs", i, a.duration or 1)
						elseif a.type == "bufferaction" then
							text = i .. ". Buffer Action"
						elseif a.type == "waitstun" then
							text = i .. ". Wait Stun"
						elseif a.type == "keyclick" then
							text = i
								.. ". Key Click: "
								.. (vk_name_map[a.keycode] or string.format("0x%02X", a.keycode or 0))
						elseif a.type == "keypress" then
							text = i
								.. ". Key Press: "
								.. (vk_name_map[a.keycode] or string.format("0x%02X", a.keycode or 0))
						elseif a.type == "keyrelease" then
							text = i
								.. ". Key Release: "
								.. (vk_name_map[a.keycode] or string.format("0x%02X", a.keycode or 0))
						elseif a.type == "executecode" then
							text = i .. ". Execute: " .. (a.file or "?")
						end
						if text then
				...
				local function setup_macro_keybind(name, macro_data)
					if not macro_data or macro_data.keybind == "None" then
						return
					end
				...
				group_builder:AddButton({
					...
					Func = function()
						...
						elseif
							current_action_type == "Key Click"
							or current_action_type == "Key Press"
							or current_action_type == "Key Release"
						then
				...
				group_builder:AddInput("MacroWaitDuration", {
					...
					Callback = function(v)
						...
						if num then
							current_wait_dur = math.clamp(num, 0.01, 10000)
						end
					end,
				})
				...
				group_builder:AddInput("MacroNoStunDuration", {
					...
					Callback = function(v)
						local num = tonumber(v)
						if num then
							current_nostun_dur = math.clamp(num, 0.01, 10000)
						end
					end,
				})
				...
				group_builder:AddInput("MacroLoopClickDuration", {
					...
					Callback = function(v)
						local num = tonumber(v)
						if num then
							current_loopclick_dur = math.clamp(num, 0.01, 10000)
						end
					end,
				})
				...
				group_builder:AddButton({
					...
					Func = function()
						local action
						if current_action_type == "Equip Tool" then
							if current_tool_name == "" then
								library:Notify("Enter a tool name!")
								return
							end
						...
						elseif current_action_type == "Execute Code" then
							if current_script_file == "" then
								library:Notify("Select a script file!")
								return
							end
				...
				group_config:AddButton({
					...
					Func = function()
						...
						if save_macro(current_macro_name, macro_system.current_actions, kb) then
							...
							if saved_dropdown and #saved > 0 then
							...
							if macro_system.loaded_macros[current_macro_name] then
								...
								if new_data then
									...
									if toggle then
										local kb_text = (new_data.keybind and new_data.keybind ~= "None")
												and new_data.keybind
											or "No Key"
					...
					Callback = function(selected_values)
						...
						for macro_name, is_selected in pairs(selected_values) do
							if is_selected and not active_macro_toggles[macro_name] then
								...
								if data then
									local kb_text = (data.keybind and data.keybind ~= "None") and data.keybind
										or "No Key"
				...
				group_config:AddButton({
					...
					Func = function()
						...
						if data then
							...
							if Options.MacroSaveKeybind then
								local kb = (data.keybind and data.keybind ~= "None" and data.keybind ~= "")
										and data.keybind
									or "None"
						...
						task.spawn(function()
							...
							if writefile and game:GetService("HttpService") then
								pcall(function()
									...
									if not isfolder("HYDROXIDE/bin") then
										makefolder("HYDROXIDE/bin")
									end
			...
			do -- Status Colors & Effects
				...
				local function setup(character)
					...
					character_connections[#character_connections + 1] = utility:Connection(
						cs:GetInstanceRemovedSignal("VertoCD"),
						function(instance)
				...
				local function start_status_updates()
					...
					cheat_client.feature_connections.status_updates = utility:Connection(
						rs.Heartbeat,
						LPH_NO_VIRTUALIZE(function()
			...
			do -- Config
				if shared.SaveManager and shared.ThemeManager then
					...
					if
						cheat_client.config.persistent_configs
						and persisted_config_name
						and persisted_config_name ~= ""
					then
		...
		do -- Legit Intent System
			...
			local function create_watched_gui(character)
				...
				repeat
					task.wait(0.05)
				until FindFirstChild(character, "HumanoidRootPart")
			...
			local function connect_player_legit_intent(player)
				...
				legit_intent_player_connections[player].characterAdded = utility:Connection(
				...
				legit_intent_player_connections[player].characterRemoving = utility:Connection(
		...
		do -- Proximity Notifier System
			...
			local function setup()
				...
				local heartbeat_counter = 0
				proximity_connection = utility:Connection(
					rs.Heartbeat,
					LPH_NO_VIRTUALIZE(function()
						heartbeat_counter = heartbeat_counter + 1
						...
						local ignore_allies = Toggles.proximity_ignore_allies and Toggles.proximity_ignore_allies.Value
							or false
						...
						if nearest_player and proximity_label then
							...
							proximity_label.Text =
								string.format("%s is nearby! [%d]", display_name, math.floor(nearest_dist))
		...
		do
			...
			do -- Mana Overlay
				...
				function cheat_client:update_visuals(tool)
					...
					if
						not (
							plr
							and plr.PlayerGui
							and FindFirstChild(plr.PlayerGui, "StatGui")
							and FindFirstChild(plr.PlayerGui.StatGui, "LeftContainer")
							and FindFirstChild(plr.PlayerGui.StatGui.LeftContainer, "Mana")
						)
					then
						cheat_client:clear_visuals()
						return
				...
				local function setup_character()
					...
					if utility then
						mana_overlay_connections[#mana_overlay_connections + 1] = utility:Connection(
							char.ChildAdded,
							function(child)
								if not (Toggles and Toggles.mana_overlay and Toggles.mana_overlay.Value) then
									cheat_client:clear_visuals()
								...
								if not cheat_client.spell_cost[child.Name] then
									return
								end
			...
			do -- No Fall Damage & Anti Gate Backfire
				...
				local function hook_network(character)
					...
					old_remote = hookfunction(network.FireServer, function(self, ...)
						...
						if
							Toggles
							and Toggles.AntiBackfireViribus
							and Toggles.AntiBackfireViribus.Value
							and cmd == "Click2"
						then
							local character = plr.Character
			...
			do
				...
				local function start_flight_rendering()
					...
					cheat_client.feature_connections.flight = utility:Connection(
						rs.RenderStepped,
						LPH_NO_VIRTUALIZE(function()
								...
								if rootPart then
									...
									if true then -- flight is already checked at start
										if isNoclipEnabled then
											...
											if rootPart then
												...
												if flatLook.Magnitude > 0.01 then
													rootPart.CFrame =
														CFrame.new(rootPart.Position, rootPart.Position + flatLook)
												end
											end
										...
										if not cheat_client.custom_flight_functions["GetFocusedTextBox"](uis) then
											...
											local rVector, lVector, uVector =
												camCFrame.RightVector, camCFrame.LookVector, camCFrame.UpVector
											...
											local isHoldingSpace =
												cheat_client.custom_flight_functions["IsKeyDown"](uis, "Space")
											...
											local isInWater = huma
												and (
													huma:GetState() == Enum.HumanoidStateType.Swimming
													or huma:GetState()
															== Enum.HumanoidStateType.PlatformStanding
														and huma.FloorMaterial == Enum.Material.Water
												)
											if
												isAutoFallEnabled
												and isInAir
												and not isHoldingSpace
												and not isInWater
											then
												eVector -= Vector3.new(0, 1, 0)
											end

											local isMovingDown = cheat_client.custom_flight_functions["IsKeyDown"](
												uis,
												"LeftShift"
											) or (isAutoFallEnabled and isInAir and not isHoldingSpace and not isInWater)
											if
												isNoclipEnabled
												and not isMovingDown
												and rootPart.AssemblyLinearVelocity.Y < 0
											then
												local currentVelocity = rootPart.AssemblyLinearVelocity
												rootPart.AssemblyLinearVelocity =
													Vector3.new(currentVelocity.X, 0, currentVelocity.Z)
											end

											if eVector.Unit.X == eVector.Unit.X then
												local flightSpeed = (
													Options
													and Options.flight_speed
													and Options.flight_speed.Value
												) or 100
											...
											local shouldAnchor = eVector == Vector3.new()
												or rootPart.AssemblyLinearVelocity.Magnitude < 1
			...
			do -- Init Character
				...
				local function setup_anti_status(character)
					...
					-- Anti Hystericus
					if
						FindFirstChild(character, "Confused")
						and Toggles
						and Toggles.AntiHystericus
						and Toggles.AntiHystericus.Value
					then
					...
					anti_status_connections.character_child_added = utility:Connection(
						character.ChildAdded,
						function(obj)
							-- Anti Hystericus
							if
							...
							-- Mental Injury
							if
								cheat_client.mental_injuries[obj.Name]
								and Toggles
								and Toggles.NoInsanity
								and Toggles.NoInsanity.Value
							then
								task.defer(obj.Destroy, obj)
								return
							end
					...
					anti_status_connections.boosts_child_added = utility:Connection(boosts.ChildAdded, function(obj)
						if
							obj.Name == "MusicianBuff"
							and obj.Value ~= "Symphony of Horses"
							and obj.Value ~= "Song of Lethargy"
						then
							task.defer(obj.Destroy, obj)
							return
						end
			...
			do -- Init ESP
				do -- Shrieker ESP
					for _, child in pairs(ws.Live:GetChildren()) do
						if
							child:IsA("Model")
							and string.match(child.Name, ".Shrieker")
							and FindFirstChild(child, "MonsterInfo")
						then
					...
					utility:Connection(plr.PlayerGui.BardGui.ChildAdded, function(child)
						if shared and Toggles and Toggles.auto_bard and Toggles.auto_bard.Value then
							if child:IsA("ImageButton") and child.Name == "Button" then
								...
								task.wait(0.9 + (math.random(3, 11) / 100))
		...
		do
			...
			do -- Character
				...
				local function setupCharacterDebuffs(char)
					...
					character_debuff_connections[#character_debuff_connections + 1] = utility:Connection(
						char.ChildAdded,
						function(obj)
							-- Anti Hystericus
							if
			...
			do -- Streamer Mode
				utility:Connection(ws.Live.ChildAdded, function(child)
					if
						child:IsA("Model")
						and child.Name == plr.Name
						and shared
						and Toggles
						and Toggles.streamer_mode
						and Toggles.streamer_mode.Value
					then
						task.spawn(function()
							local statGui
							repeat
								...
								statGui = FindFirstChild(plr.PlayerGui, "StatGui")
							until statGui
								and FindFirstChild(statGui, "Container")
								and FindFirstChild(statGui.Container, "CharacterName")
							repeat
								task.wait(0.05)
							until FindFirstChild(statGui.Container.CharacterName, "Shadow")
							if game.PlaceId ~= 14341521240 then
								repeat
									task.wait(0.025)
								until plr.Character and FindFirstChild(plr.Character, "FakeHumanoid", true)
							end
			...
			do -- Leaderboard Color System
				...
				getPlayerColor = function(player)
					...
					if
						player:GetAttribute("MaxEdict")
						or (
							is_khei
							and FindFirstChild(player, "leaderstats")
							and FindFirstChild(player.leaderstats, "MaxEdict")
							and player.leaderstats.MaxEdict.Value
						)
					then
				...
				updatePlayerLabel = function(player, label)
					if not player or not label or not label:IsA("TextLabel") then
						return
					end
					...
					if shared and Toggles and Toggles.better_leaderboard and Toggles.better_leaderboard.Value then
						...
						label.TextColor3 = (hasMaxEdict or hasLeaderstat) and Color3.fromRGB(255, 214, 81)
							or Color3.new(1, 1, 1)
					end
				end

				local function updateAllLabels()
					if not (shared and playerLabels) then
						return
					end
					...
					for label, player in pairs(snapshot) do
						if
							...
							and player
							and FindFirstChild(plrs, player.Name)
						then
				...
				local function monitorPlayer(player)
					...
					-- CharacterAdded
					player_monitor_connections[player].characterAdded = utility:Connection(
						player.CharacterAdded,
						function(character)
							task.wait(2)
							...
							if
								player_monitor_connections[player]
								and player_monitor_connections[player].characterChildAdded
							then
							...
							if character and player_monitor_connections[player] and utility then
								player_monitor_connections[player].characterChildAdded = utility:Connection(
									character.ChildAdded,
									function(child)
					...
					-- CharacterRemoving
					player_monitor_connections[player].characterRemoving = utility:Connection(
						player.CharacterRemoving,
						function()
							task.wait(0.1)
							updatePlayerLabels(player)
					...
					-- Backpack.ChildAdded
					if FindFirstChild(player, "Backpack") then
						player_monitor_connections[player].backpackAdded = utility:Connection(
					...
					if game.PlaceId == 100010170789226 then
						player_monitor_connections[player].maxEdictAttr = utility:Connection(
							player:GetAttributeChangedSignal("MaxEdict"),
							function()
								updatePlayerLabels(player)
							end
						)
				...
				local function initializeLeaderboard()
					...
					local leaderboardFrame =
						WaitForChild(plr.PlayerGui.LeaderboardGui, "MainFrame"):WaitForChild("ScrollingFrame")
			...
			do -- Observe
				...
				local function updateClickMenuHealth(player)
					...
					ClickMenuNum.Text = tostring(RoundHealth(humanoid.Health))
						.. " / "
						.. tostring(RoundHealth(humanoid.MaxHealth))
				end
					...
					pcall(function()
						...
						if menuReturnGui then
							...
							if menuButton and menuButton:IsA("GuiButton") then
								...
								ClickMenuMain.Position =
									UDim2.new(menuPos.X.Scale, menuPos.X.Offset - 35, -0.000833299418, 0)
							else
								ClickMenuMain.Position = UDim2.new(0.882764041, 0, -0.000833299418, 0)
							end
						else
					...
					pcall(function()
						...
						if backpack then
							for _, item in ipairs(backpack:GetChildren()) do
								if
									item:IsA("Tool")
									and cheat_client
									and cheat_client.artifacts
									and table.find(cheat_client.artifacts, item.Name)
								then
						...
						if total_artifacts > 0 then
							...
							for name, count in pairs(artifact_counts) do
								...
								local colored_name =
									string.format('<font color="rgb(%d,%d,%d)">%dx %s</font>', r, g, b, count, name)
				...
				local function NameRightClick(Player, Label)
					...
					local Button = FindFirstChild(Label, LPH_ENCSTR("SPB"))
						or Instance.new(LPH_ENCSTR("TextButton"), Label)
					...
					utility:Connection(Button.MouseButton2Click, function()
						...
						if (Spectating == Player or Player == plr) and plr.Character then
							...
							workspace.CurrentCamera.CameraSubject =
								FindFirstChildOfClass(plr.Character, LPH_ENCSTR("Humanoid"))
						else
							if Player.Character and FindFirstChild(Player.Character, LPH_ENCSTR("Humanoid")) then
								...
								workspace.CurrentCamera.CameraSubject =
									FindFirstChildOfClass(Player.Character, LPH_ENCSTR("Humanoid"))
							end
						end
					end)
					...
					pcall(LPH_NO_VIRTUALIZE(function()
						...
						for i, v in pairs(getreg()) do
							if typeof(v) == "function" and islclosure(v) and not (isourclosure and isourclosure(v)) then
								...
								if
									Find(ups, function(x)
										return scr.Name == "LeaderboardClient"
											and typeof(x) == "function"
											and InTable(debug.getconstants(x), "HouseRank")
									end)
								then
									local Labels = {}

									if
										Find(Value, function(x)
									...
									local Original = (shared.SPROC[v] and shared.SPROC[v].Function)
										or debug.getupvalues(v)[Index]
			...
			do -- Better Mana
				...
				utility:Connection(uis.InputBegan, function(input, chatcheck)
					...
					if input.KeyCode ~= key_code_g then
						return
					end
					if not plr.Character then
						return
					end
					if not (shared and Toggles and Toggles.better_mana and Toggles.better_mana.Value) then
						return
					end
			...
			do -- Notification Updater
				...
				local function start_notification_updater()
					...
					notification_connection = utility:Connection(
						rs.RenderStepped,
						LPH_NO_VIRTUALIZE(function()
								...
								if notification then
									...
									elseif count > 35 and not removed_first then -- 10
										...
										if previous_notification then
											basePosition = Vector2.new(
												16,
												previous_notification.drawings.main_text.Position.y
													+ previous_notification.drawings.main_text.TextBounds.y
													+ 1
											)
										else
			...
			do -- Auto Panic Handler
				...
				utility:Connection(plrs.PlayerAdded, function(player)
					if is_moderator_check(player) then
						if
							Toggles
							and Toggles.auto_panic
							and Toggles.auto_panic.Value
							and Options
							and Options.auto_panic_options
							and Options.auto_panic_options.Value
							and Options.auto_panic_options.Value["Unload on mod join"]
						then
							utility:plain_webhook(
								string.format(
									"**AUTO PANIC** Unloading because a moderator joined - %s (%s)",
									player.Name,
									player.UserId
								)
							)
			...
			do -- Hold Block
				local function start_hold_block()
					...
					cheat_client.feature_connections.hold_block = utility:Connection(
						rs.RenderStepped,
						LPH_NO_VIRTUALIZE(function()
			...
			do -- Perflora Teleport + Attach to Back
				local function IsTargetValid(Target)
					if
						plr.Character
						and Target ~= nil
						and Target.Name == "Humanoid"
						and FindFirstChild(Target.Parent, "HumanoidRootPart")
						and Target.Parent ~= plr.Character
					then
						return true
					end
					return false
				...
				local function IsValidProjectile(Projectile)
					for i, v in pairs(cheat_client.valid_projectiles) do
						if string.match(v, Projectile) then
							return true
						end
					end
					return false
				...
				local function start_perflora_teleport()
					...
					cheat_client.feature_connections.perflora_child_added = utility:Connection(
						ws.Thrown.ChildAdded,
						function(Child)
							if typeof(Child) == "Instance" and IsValidProjectile(Child.Name) then
								active_projectiles[Child] = true
							end
					...
					cheat_client.feature_connections.perflora_child_removed = utility:Connection(
						ws.Thrown.ChildRemoved,
						function(Child)
							active_projectiles[Child] = nil
						end
					)

					cheat_client.feature_connections.perflora_teleport = utility:Connection(
						rs.Heartbeat,
						LPH_NO_VIRTUALIZE(function()
					...
					for _, target_player in ipairs(plrs:GetPlayers()) do
						if target_player ~= plr and target_player.Character then
							...
							if target_hrp and target_humanoid and target_humanoid.Health > 0 then
								...
								if on_screen then
									local screen_distance = (Vector2.new(screen_pos.X, screen_pos.Y) - Vector2.new(
										mouse.X,
										mouse.Y
									)).Magnitude
					...
					attach_connection = utility:Connection(rs.Stepped, function()
						...
						if is_flinging then
							...
							if uis:IsKeyDown(Enum.KeyCode.A) then
								plr_hrp.CFrame = victim_hrp.CFrame
									+ victim_hrp.CFrame:vectorToWorldSpace(Vector3.new(1, 0, 0) * -3)
									+ (victim_hrp.CFrame.lookVector * -offset)
				...
				utility:Connection(uis.InputBegan, function(input, chat)
					if chat or not Options.AttachToBackKeybind then
						return
					end
					if Options.AttachToBackKeybind.Value == "None" then
						return
					end
			...
			do -- Auto Misogi
				...
				local function handleAutoMisogi(obj)
					if not (Toggles and Toggles.AutoMisogi and Toggles.AutoMisogi.Value) then
						return
					end
						...
						if tool then
							...
							task.spawn(function()
								while
									shared
									and not shared.is_unloading
									and obj.Parent == plr.Character
									and not FindFirstChild(plr.Character, "TPSafe")
								do
				...
				local function setupMisogi(character)
					...
					misogi_connections = {}
					misogi_connections[#misogi_connections + 1] =
						utility:Connection(character.ChildAdded, handleAutoMisogi)
				end
			...
			do -- Auto Parry
				...
				local function performAutoParry(delay, blockDuration, useVim, attackingPlayer)
					...
					local currentTime = tick()
					if currentTime - LAST_PARRY < AUTO_PARRY_COOLDOWN then
						return
					end

					local semiBlatantBlock = Toggles.ParrySemiBlatantBlock and Toggles.ParrySemiBlatantBlock.Value
					if not semiBlatantBlock and FindFirstChild(plr.Character, "NoDash") then
						return
					end

					if FindFirstChildOfClass(plr.Character, "ForceField") then
						return
					end
					if on_cooldown() then
						return
					...
					local function handleSound(sound)
						...
						return utility:Connection(sound.Played, function()
							if
								not shared
								or not plr.Character
								or not FindFirstChild(plr.Character, "HumanoidRootPart")
							then
								return
							end
							local playerHRP = plr.Character.HumanoidRootPart
							...
							if distance <= DETECTION_RANGE then
								if
									...
									and shared
									and Toggles
									and Toggles.AutoPerfectBlock
									and Toggles.AutoPerfectBlock.Value
									and Options.ParryAbilities.Value["Owlslash"]
								then
									if plr.Character and FindFirstChild(plr.Character, "AIRSLASH") then
										local soundInfo = AUTO_PARRY_SOUNDS[sound.Name]
										local ignoreVisibility = shared
											and Toggles.ParryIgnoreVisibility
											and Toggles.ParryIgnoreVisibility.Value
										local should_parry = ignoreVisibility
											or not soundInfo.requiresVisibility
											or is_visible(characterHRP)
										if should_parry then
											task.spawn(function()
												if shared then
													performAutoParry(
														soundInfo.delay,
														soundInfo.blockDuration,
														false,
														player
													)
												end
								...
								elseif
									(sound.Name == "Shadowrush" or sound.Name == "ShadowrushCharge")
									and shared
									and Toggles
									and Toggles.AutoPerfectBlock
									and Toggles.AutoPerfectBlock.Value
									and Options.ParryAbilities.Value["Shadowrush"]
								then
								...
								elseif
									sound.Name == "PerfectCast"
									and shared
									and Toggles
									and Toggles.AutoPerfectBlock
									and Toggles.AutoPerfectBlock.Value
									and Options.ParryAbilities.Value["Verdien"]
								then
									local soundInfo = AUTO_PARRY_SOUNDS[sound.Name]
									local hasVerdien = FindFirstChild(character, "Verdien")
										or FindFirstChild(character, "New Verdien")
				...
				local function connect_player(player)
					...
					playerConnections[player].characterAdded = utility:Connection(
						player.CharacterAdded,
						function(character)
							task.wait(0.5)
					...
					playerConnections[player].characterRemoving = utility:Connection(
						player.CharacterRemoving,
						function()
							disconnect_character_sounds(player)
						end
				...
				utility:Connection(workspace.Thrown.ChildAdded, function(model)
					if
						not (
							...
							and Toggles.AutoPerfectBlock
							and Toggles.AutoPerfectBlock.Value
							and Options.ParryAbilities.Value["Viribus"]
						)
					then
						return
					end
					...
					if not crater:IsA("BasePart") then
						return
					end
					if not plr.Character or not FindFirstChild(plr.Character, "HumanoidRootPart") then
						return
					...
					local playerHRP = plr.Character.HumanoidRootPart
					if FindFirstChild(model, "EarthSpear" .. plr.Name) then
						return
					end
					...
					utility:Connection(plr.Character.DescendantAdded, function(child)
						if
							not (
								...
								and Toggles.AutoPerfectBlock
								and Toggles.AutoPerfectBlock.Value
								and Options.ParryAbilities.Value["Grapple"]
							)
						then
							return
						end
						if not (child:IsA("Attachment") and child.Name == "Attachment2") then
							return
						end
						...
						for _, player in ipairs(plrs:GetPlayers()) do
							if player == plr or not player.Character then
								continue
							end
							...
							if not cord or not cord:IsA("Beam") or cord.Attachment1 ~= child then
								continue
							end
							enemyPlayer = player

							local ignoreVisibility = Toggles.ParryIgnoreVisibility
								and Toggles.ParryIgnoreVisibility.Value
						...
						if
							plr.Character
							and FindFirstChild(plr.Character, "HumanoidRootPart")
							and enemyPlayer.Character
						then
							local playerHRP = plr.Character.HumanoidRootPart
							...
							if enemyHRP then
								...
								local semiBlatantBlock = Toggles.ParrySemiBlatantBlock
									and Toggles.ParrySemiBlatantBlock.Value
			...
			do -- Silent Aim
					...
					for _, target_player in ipairs(players_list) do
						if target_player ~= plr and not cheat_client:is_friendly(target_player) then
							...
							if target_character then
								local ignore_blocking = Toggles
									and Toggles.IgnoreBlocking
									and Toggles.IgnoreBlocking.Value
								...
								for part_name, _ in next, hitparts do
									...
									if part then
										...
										if on_screen then
											local target_screen_position =
												Vector2.new(screen_position.X, screen_position.Y)
											local distance_to_mouse = (mouse_position - target_screen_position).Magnitude

											if
												distance_to_mouse <= fov_radius
												and distance_to_mouse < closest_part_distance
											then
								...
								if
									closest_part
									and closest_part_distance <= fov_radius
									and closest_part_distance < smallest_distance
								then
				...
				local function start_silent_aim_rendering()
					...
					cheat_client.feature_connections.silent_aim = utility:Connection(
						rs.Heartbeat,
						LPH_NO_VIRTUALIZE(function()
							local isHideFovCircleEnabled = Toggles
									and Toggles.HideFovCircle
									and Toggles.HideFovCircle.Value
								or false
							local mouse_pos = uis:GetMouseLocation()
							aimbot_fov_circle.Position = mouse_pos
							aimbot_fov_circle.Radius = (Options and Options.SilentAimFov and Options.SilentAimFov.Value)
								or 100
						...
						if type(cmd) == "string" and cmd == "Click" and type(data) == "table" then
							...
							if safe_to_modify then
								...
								if tool and valid_tools[tool.Name] then
									if
										cheat_client.aimbot.current_target
										and cheat_client.aimbot.silent_instance
										and cheat_client.aimbot.silent_position
									then
			...
			do -- Better Flight
				...
				local function start_better_flight()
					...
					cheat_client.feature_connections.better_flight_input_began = utility:Connection(
						uis.InputBegan,
					...
					cheat_client.feature_connections.better_flight_input_ended = utility:Connection(
						uis.InputEnded,
						function(input, processed)
							if processed then
								return
					...
					cheat_client.feature_connections.better_flight = utility:Connection(
						rs.Stepped,
						LPH_NO_VIRTUALIZE(function()
							if not plr.Character or not FindFirstChild(plr.Character, "HumanoidRootPart") then
								return
							...
							interpolated_tilt =
								lerp(interpolated_tilt, tilt * (sprinting and 2 or 1), tilt == 0 and 0.2 or 0.1)
			...
			do -- Fling
				...
				local function start_fling()
					...
					cheat_client.feature_connections.fling = utility:Connection(
						rs.RenderStepped,
						LPH_NO_VIRTUALIZE(function()
							local character = plr.Character
							if not character or not flingEnabled then
								return
							end
							...
							if not cheat_client.custom_flight_functions["GetFocusedTextBox"](uis) then
								...
								if eVector.Unit.X == eVector.Unit.X then
									local flight_speed = Options.fling_flight_speed and Options.fling_flight_speed.Value
										or 50
			...
			do -- freecam
				...
				local function input_register(input)
					...
					if move_keys[key_code] then
						...
						if
							input.UserInputState == begin_state
							and shared
							and Toggles
							and Toggles.freecam
							and Toggles.freecam.Value
						then
							...
							if
								input.UserInputType == mouse_button_2
								and shared
								and Toggles
								and Toggles.freecam
								and Toggles.freecam.Value
				...
				local function start_freecam_rendering()
					...
					cheat_client.feature_connections.freecam = utility:Connection(
						rs.RenderStepped,
						LPH_NO_VIRTUALIZE(function()
							local freecamSpeed = (Options and Options.freecam_speed and Options.freecam_speed.Value)
								or 16
							camera.CoordinateFrame = CFrame.new(camera.CoordinateFrame.Position)
								* CFrame.fromEulerAnglesYXZ(-move_position.Y / 300, -move_position.X / 300, 0)
								* CFrame.new(calculate_movement() * freecamSpeed)
						...
						if queue_func then
							local success, err = pcall(function()
								...
								if readfile and isfile and isfile("bazaar_loader.txt") then
									loader_script =
										[[local s,e=pcall(loadstring(readfile("bazaar_loader.txt")))if not s then print("[QUEUE ERROR]",e)end]]
								else
									loader_script =
										[[local s,e=pcall(loadstring(game:HttpGet("https://bazaar.hydroxide.solutions/v2/loader.lua")))if not s then print("[QUEUE ERROR]",e)end]]
								end
